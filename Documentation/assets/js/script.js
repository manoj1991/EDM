...
$(document).ready(function() {
	...
	if(!badIE){
		...
		$('a[href^=#]').bind('click touchstart',function(){
			...
			goTo(hash);
			if(!$(this).parents('ul').hasClass('submenu') && !$(this).parents('li').find('ul').hasClass('submenu')){
				//alert(88)
				$('.submenu').slideUp();
				$(this).parents('li').siblings().find('.dropdown').removeClass('rotate');
			}
			else if($(this).parents('li').find('ul').hasClass('submenu')){
				//alert(88)
				$(this).parents('li').find('.dropdown').addClass('rotate');
				$(this).parents('li').find('.submenu').slideDown();
				$(this).parents('li').find('.submenu li').find('a').removeClass('current');
			}
			if($(window).width()<768){
				$('#documenter_sidebar').removeClass('open');
			}
			if($(this).parent().parent().hasClass('submenu')){
				$(this).parent().parents('li').siblings().find('a').removeClass('current');
	...
	$('#documenter_sidebar ul .dropdown').click(function(){
